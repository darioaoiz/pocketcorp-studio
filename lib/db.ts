...
/**
 * Media lives under `storage/` by default, but can be redirected — e.g. to a
 * folder synced by Google Drive or Dropbox — via POCKETCORP_MEDIA_DIR. Only
 * media is safe to move this way: each file is written once and never
 * touched again, which is exactly what cloud-sync clients handle safely.
 * The database is NOT redirectable by design — it's written to constantly,
 * and a sync client can corrupt a SQLite file mid-write.
 */
export const MEDIA_DIR = process.env.POCKETCORP_MEDIA_DIR
  ? path.resolve(process.env.POCKETCORP_MEDIA_DIR)
